...
export interface EnhancedRequestHandlerExtra<TDeps = any> extends RequestHandlerExtra<any, any> {
  injected: TDeps;
}
        ...
        const enhanced = async (request: any, extra: any) => {
          try {
            ...
            return {
              content: [{ 
                type: "text", 
                text: error instanceof Error ? error.message : "Dependency injection failed" 
              }],
              isError: true
            };
          }
